 ...
 * 图片延迟加载
 * 使用方法：
 * import Lazy from '../lib/klazy';
 * const _lazy = new Lazy();
 * _lazy.start(dv); // 注意，这个dv是滚动的层，错了，无法触发加载，sui 就是内容层！
...
// options
const _opts = {
  normal: 'nor', // 'data-normal'
  retina: 'ret', // 'data-retina',
  srcset: 'set', // 'data-srcset',
  threshold: 0
};
...
const _srcset = document.body.classList.contains('srcset') || 'srcset' in document.createElement('img');
...
function update(root) {
  if (root)
    _nodes = Array.prototype.slice.call(root.querySelectorAll(`[${_opt.normal}]`));
  else
    _nodes = Array.prototype.slice.call(document.querySelectorAll(`[${_opt.normal}]`));
  return this;
}